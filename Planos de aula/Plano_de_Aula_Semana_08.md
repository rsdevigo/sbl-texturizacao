...
- Asset 01 no 3D Coat com material PBR iniciado: Color (base fotográfica ou estilizada da Semana 6), Roughness e Normal (via Depth) ativos, com máscaras de AO/Curvature aplicadas — Semana 7
- Asset 02 no 3D Coat com material PBR avançado (mínimo: Color + Normal) — Semana 7
- Trilha visual definida e consistente entre os dois assets (fotorrealista ou estilizada)
...
6. (Crítica formal) Apresentar e defender oralmente as decisões de material tomadas durante as Semanas 6–7, utilizando vocabulário técnico correto e a autoavaliação como ponto de partida, justificando a trilha visual escolhida.
...
| C2 — Direção Artística | Coerência da paleta e proposta estética ao longo dos dois assets apresentados; capacidade de verbalizar as escolhas visuais durante a crítica, incluindo a justificativa da trilha (fotorrealista/estilizada) |
...
- Arquivos `.3b`/`.3dc` dos Assets 01 e 02 com o material iniciado na Semana 7
- Arquivos `.blend` dos Assets 01 e 02 com UV das semanas anteriores
...
Objetivo: a turma já domina a mecânica básica do 3D Coat desde a Semana 7 — esta mini-aula não reapresenta a ferramenta, ela aprofunda duas coisas que ainda faltam: o canal Subsurface (para quem tiver elementos orgânicos no kit) e a lógica de **combinar** camadas de fontes diferentes (base fotográfica/Smart Material + máscara de AO/Curvature + pintura manual de detalhe) em um resultado final coerente, fechando com o pacote completo de exportação.

**Desenvolvimento:**

Abrir com a seguinte pergunta para a turma: *"Vocês já têm Color, Roughness e Normal ativos desde a semana passada. O que ainda falta para esse material estar pronto para ir para a Unity?"* Conduzir a turma a perceber que falta: (1) o Metallic, ainda não trabalhado a fundo; (2) mais uma camada de detalhe intencional sobre o que já existe; (3) a exportação de tudo isso como imagem real.
...
3. Rapidamente, mostrar em um segundo asset preparado (caixa metálica): Fill com branco puro (Metallic = 1), com preto pintado nas áreas de ferrugem/tinta exposta usando a mesma lógica de máscara por Curvature.
...
5. Abrir a pasta no explorador: mostrar os quatro arquivos PNG gerados — `Asset_Albedo.png`, `Asset_Roughness.png`, `Asset_Metallic.png`, `Asset_Normal.png`.

**Passo 5 — Conexão no Blender (3 min):**
1. Mudar para o Blender.
2. No Shader Editor, criar um material novo e limpo (sem nós procedurais remanescentes): `[Nome]_Asset_3DCoat_Final`.
3. Adicionar quatro nós `Image Texture`:
   - `Asset_Albedo.png` → `Base Color` (Color Space: **sRGB**, padrão)
   - `Asset_Roughness.png` → `Roughness` (Color Space: **Non-Color**)
   - `Asset_Metallic.png` → `Metallic` (Color Space: **Non-Color**)
   - `Asset_Normal.png` → através de um nó `Normal Map` (`Shift+A → Vector → Normal Map`) → entrada `Normal` do Principled BSDF (Color Space do Image Texture: **Non-Color**)
...
**Refinamento do Asset 01, canal Metallic e primeira exportação de teste**

**Consigna entregue verbalmente:**

> *"O objetivo deste estúdio é ter o Asset 01 com os quatro canais refinados — Color com uma camada de detalhe adicional, Roughness revisado, Metallic corretamente preenchido — e uma primeira exportação de teste dos quatro mapas. Não precisa ser a versão final: o segundo encontro tem mais 60 minutos de estúdio, incluindo a reconexão completa no Blender. Antes de sair daqui, quero ver o Metallic preenchido com lógica física e pelo menos uma exportação de teste na pasta do projeto."*

**Atividade estruturada:**

**Etapa 1 — Refinamento do Color (≈15 min):**
1. Abrir o arquivo `.3b`/`.3dc` do Asset 01 da Semana 7.
...
4. Pasta: `[Nome]_Asset01_Mapas_S08/`.
...
1. **O estudante apresenta** (2 min):
   - Projeta o asset com o material no Render Room do 3D Coat ou no viewport Rendered do Blender.
...
> *"Sessenta minutos para três objetivos: (1) refinamento final das camadas do Asset 01 no 3D Coat, incorporando o que saiu na crítica; (2) exportação final dos quatro mapas PBR — Albedo, Metallic, Roughness e Normal; (3) reconexão completa no Blender, sem nenhum nó procedural remanescente. Quem terminar o Asset 01 e tiver tempo: repete o fluxo completo no Asset 02."*
...
1. Abrir o `.blend` do Asset 01.
2. Criar um material novo e limpo: `[Nome]_Asset01_3DCoat_Final`.
3. Adicionar quatro nós `Image Texture`:
   - `Asset_Albedo.png` → `Base Color` (sRGB)
   - `Asset_Roughness.png` → `Roughness` (Non-Color)
   - `Asset_Metallic.png` → `Metallic` (Non-Color)
   - `Asset_Normal.png` → nó `Normal Map` → `Normal` do Principled BSDF (Image Texture em Non-Color)
...
5. Salvar: `[Nome]_Asset01_3DCoat_S08.blend`.
...
**Bloco 4 — Repetição no Asset 02 (se houver tempo — ≈5 min):**
Para estudantes que terminaram o fluxo completo do Asset 01:
1. Repetir refinamento, exportação e reconexão para o Asset 02.
2. Salvar como `[Nome]_Asset02_3DCoat_S08.blend`.
...
- Para estudantes avançados: *"Compara o Asset 01 e o Asset 02 lado a lado no Blender. Os dois materiais comunicam claramente o mesmo universo visual, mesmo sendo superfícies diferentes?"*
...
|---|---|---|
| Asset 01 no 3D Coat com Color refinado (base + máscara + camada de detalhe) | C5 — Texturização | Verificar no painel de Layers/Render Room: há pelo menos três camadas contribuindo para o Color, com variação visível |
...
|---|---|---|
| Arquivo do 3D Coat com Asset 01 refinado (Color, Roughness, Metallic, Normal) | `.3b`/`.3dc` com sufixo `_Asset01_3DCoat_S08` | Até o fim do segundo encontro |
...
| Arquivo Blender com material final configurado usando os quatro mapas do 3D Coat, sem nós procedurais remanescentes | `.blend` com sufixo `_Asset01_3DCoat_S08` | Até o fim do segundo encontro |
...
> **Nota:** A entrega dos mapas do Asset 02 é opcional nesta semana — será o foco das Semanas 9 e 10. Estudantes que iniciaram o fluxo completo no Asset 02 devem salvar o arquivo `.3b` mesmo que incompleto, para continuar na Semana 9.
