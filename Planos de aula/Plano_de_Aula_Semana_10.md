...
**Tema da semana:** Uso de Stencils em texturização  
...
## O que já foi ministrado (Semana 9 — não repetir)

Os estudantes chegam com:

- Hero Asset Referência finalizado com camadas de desgaste: EdgeWear, Dirt e Scratches pintados no 3D Coat, mapas PBR exportados e conectados no Blender — Semana 9
- Render comparativo antes/depois do Hero Asset Referência (S08 vs. S09) salvo — Semana 9
- Para quem já recebeu Assets Secundários de Modelagem 3D e Level Design nesta fase: arquivo(s) importado(s) no 3D Coat com ao menos as camadas base dos três canais (Color, Roughness, Metallic) — recebimento variável conforme o cronograma de entrega da disciplina de Modelagem 3D, não necessariamente concluído até esta semana
- Moodboard atualizado com referências do tema do kit — Semana 1 (atualização contínua)

> **Nota de transição — início da fase de Kit Modular:** A Semana 9 desenvolveu a linguagem do desgaste por pintura livre — edge wear, dirt, scratches — e encerrou a Unidade II inteiramente dedicada ao Hero Asset Referência. A partir desta semana abre a Unidade III: o Kit Modular propriamente dito começa a ganhar corpo. Modelagem 3D e Level Design já entregou (ou está entregando, ao longo desta fase) os **Assets Secundários** (2–3 peças com ciclo de produção abreviado) e os **Assets de Repetição** (2–4 elementos modulares como paredes, vigas e molduras, que só entrarão tecnicamente em cena na Semana 14 via Trim Sheet). O Hero Asset Referência não desaparece — ele continua existindo e será incorporado ao kit mais adiante (por exemplo, no Texture Atlas da Semana 13) — mas deixa de ser o único foco da disciplina.
>
> Esta semana introduz uma nova forma de adicionar detalhe, o stencil, e a aplica em dois frentes possíveis: (1) o refinamento do Hero Asset Referência, aprofundando símbolos, rachaduras e corrosão com mais precisão do que a pintura livre permite; e (2) para quem já tem em mãos os primeiros Assets Secundários, o início da aplicação da mesma linguagem visual — os mesmos stencils, a mesma paleta, o mesmo nível de desgaste — a essas novas peças, começando a dar identidade de conjunto ao kit. Se a pintura livre é como desenhar à mão, o stencil é como usar um carimbo com controle total de posição, escala e ângulo. Não são técnicas concorrentes — são complementares.

> **Atenção à Semana 11 (Crítica FORMAL — CF3):** Esta é a última semana antes da crítica formal da Unidade III. A CF3 avalia o bake de Normal/AO/Curvature aplicado a um asset do Kit Modular (provavelmente um Asset Secundário, com o par low-poly/high-poly já preparado por Modelagem 3D). O estudante deve sair do segundo encontro desta semana com o Hero Asset Referência refinado e, se já tiver Assets Secundários em mãos, com ao menos um deles com a linguagem visual do kit iniciada. O fechamento desta aula deve deixar isso explícito.
...
6. Para quem já tem Assets Secundários em mãos: reconhecer e reproduzir a **mesma linguagem visual** (tipo de stencil, paleta de cor, nível de desgaste) já estabelecida no Hero Asset Referência, iniciando a coerência de conjunto do Kit Modular.
7. Exportar os mapas atualizados do(s) asset(s) trabalhado(s) na semana e conectar no Blender para verificação visual.

---

## Critérios da Rubrica Mestre ativos nesta semana

> ⚠️ **Semana sem crítica formal — nenhuma nota é atribuída.** O professor observa e registra evidências nos critérios abaixo para calibrar a avaliação da CF3 (Semana 11). O estudante recebe feedback oral durante o estúdio.
...
| C2 — Direção Artística | O tipo de stencil escolhido é coerente com o tema do kit? (Rachaduras para pedra medieval; corrosão para sci-fi; grafites para pós-apocalíptico). Para quem trabalhou Assets Secundários: a linguagem visual aplicada a eles é reconhecivelmente a mesma do Hero Asset Referência? |
...
- Arquivo do Hero Asset Referência de cada estudante no 3D Coat (`.3b`, finalizado na S09)
- Para quem já recebeu Assets Secundários: arquivo(s) `.3b` com camadas base ou em branco, a depender do progresso do recebimento
...
> **Preparação do arquivo de demonstração:** O asset de demonstração deve ser de pedra (ou material genérico que permita rachaduras) com as três camadas base já pintadas. Não adicionar desgaste da S09 — o estudante precisa ver que o stencil pode trabalhar tanto sobre uma base limpa (como estará um Asset Secundário recém-chegado) quanto sobre um material já desgastado (como está o Hero Asset Referência). Usar o mesmo asset da S09 se possível, para que a turma perceba a continuidade.

> **Verificação prévia à aula:** Confirmar com a coordenação de Modelagem 3D e Level Design quais estudantes já receberam Assets Secundários nesta fase e quais ainda não. Isso não muda o conteúdo da aula, mas define quem segue o Caminho A (só Hero Asset Referência) e quem segue o Caminho B (Hero Asset Referência + Asset Secundário) na Produção em Estúdio — ver adiante.
...
**Stencil** é uma máscara estática projetada sobre o asset. Não importa onde você pinta — o stencil permanece fixo no espaço 3D, funcionando como um gabarito. Você controla a posição, escala e rotação do gabarito, e então pinta sobre ele. O resultado é preciso e reproduzível em qualquer área do asset — inclusive em assets diferentes, o que é exatamente o que torna o stencil útil para dar identidade de conjunto a um kit modular.
...
- Precisa ser **reproduzido em múltiplas faces** do mesmo asset ou em **assets diferentes do kit** (padrão de tijolos, marca de fabricante, grade metálica — este último uso é o que conecta visualmente o Hero Asset Referência aos Assets Secundários)
...
**5. Stencil como ferramenta de coerência de kit**

Um ponto novo nesta semana, específico do início do Kit Modular: o mesmo stencil (ou a mesma família de stencils) aplicado a mais de um asset do kit, com a mesma paleta de cor e o mesmo tipo de resposta em Roughness, é o que começa a fazer duas peças diferentes parecerem que pertencem ao mesmo mundo. *"Se o Hero Asset Referência de vocês tem rachaduras de um certo tipo, com uma certa cor de interior exposto, e o Asset Secundário que chegou de Modelagem 3D é do mesmo material, esse é o mesmo stencil e a mesma paleta — não um novo experimento. A coerência visual do kit começa exatamente aqui, numa decisão pequena de reaproveitar o que já funciona."*
...
5. Voltar ao 3D Coat: importar o stencil criado e mostrar que ele funciona imediatamente. *"Em menos de 2 minutos, qualquer foto com contraste vira stencil. A internet inteira de referências de superfície é uma biblioteca de stencils esperando para ser convertida — e o mesmo stencil que vocês acabaram de criar pode ser reaplicado depois em qualquer outro asset do kit que peça o mesmo tipo de detalhe."*
...
> **Nota do professor:** Não é necessário demonstrar stencil em múltiplos canais além de Color e Roughness — o estudante pode explorar o canal Metallic no estúdio se quiser. O foco da demonstração é o workflow completo: carregar → posicionar → aplicar em ao menos dois canais → integrar. A criação no Krita é rápida (4 min) mas de alto valor pedagógico — mostra que qualquer referência fotográfica pode se tornar ferramenta de trabalho, reutilizável em todo o kit.

---

### Produção em Estúdio — 50 minutos

**Refinamento do Hero Asset Referência e, quando aplicável, início dos Assets Secundários**

**Consigna entregue verbalmente:**

> *"Cinquenta minutos de estúdio. O ponto de partida depende de onde cada um está no kit. Quem ainda só tem o Hero Asset Referência: hoje é dia de aprofundá-lo com stencil — símbolos, rachaduras, corrosão, o que o tema pedir. Quem já recebeu os primeiros Assets Secundários de Modelagem 3D: depois de aplicar pelo menos um stencil no Hero Asset Referência, comecem a levar a mesma linguagem visual para o Asset Secundário — mesmo tipo de stencil, mesma paleta, mesmo nível de desgaste. Antes de abrir qualquer stencil, olhem o moodboard e respondam: 'Que tipo de detalhe específico esse asset precisa que pintura livre não entregaria com precisão?' Se a resposta for 'nenhum', talvez o asset não precise de stencil agora — use pintura livre. Antes de sair desta sessão: ao menos um stencil aplicado em dois canais (Color e Roughness), com as bordas integradas, no Hero Asset Referência. Screenshot do processo com o painel de Stencils visível."*

**Atividade estruturada:**

**Etapa 1 — Verificação do estado dos assets (≈5 min):**
1. Abrir o arquivo `.3b` do Hero Asset Referência no 3D Coat.
2. Abrir o Render Room por 1 minuto: avaliar o estado atual. Identificar: *que detalhe específico o tema pede que este asset ainda não tem?*
3. Para quem já recebeu Asset(s) Secundário(s): abrir também o(s) arquivo(s) correspondente(s) e verificar se as camadas base (Color, Roughness, Metallic — Fill) já existem. Se ainda não existirem, será necessário criar um Fill simples antes de aplicar qualquer stencil (ver Caminho B, Etapa 3).

**Etapa 2 — Seleção e carregamento do stencil no Hero Asset Referência (≈5 min):**
...
**Etapa 3 — Aplicação do stencil no Hero Asset Referência em múltiplos canais (≈20 min):**
...
**Etapa 4 — A partir daqui, dois caminhos (≈15 min restantes):**

**Caminho A — Estudante ainda só com o Hero Asset Referência:**
...
3. Registro de processo: screenshot do Paint Room com o painel de Stencils visível e as camadas nomeadas; screenshot do Render Room close-up; screenshot do Render Room afastado. Salvar: `[Nome]_HeroAsset_S10.3b`.

**Caminho B — Estudante que já recebeu Asset(s) Secundário(s):**
1. Abrir o arquivo do Asset Secundário. Se as camadas base ainda não existem, criar Fill de Color, Roughness e Metallic primeiro (≈5 min de trabalho rápido — o estudante já fez isso no Hero Asset Referência em semanas anteriores).
2. Aplicar **o mesmo stencil** usado no Hero Asset Referência (ou um da mesma família temática) no Asset Secundário, repetindo o fluxo de Color + Roughness.
3. Usar a mesma paleta de cor do interior exposto e o mesmo critério de intensidade que foi usado no Hero Asset Referência — o objetivo aqui não é criatividade nova, é repetição consciente da linguagem já estabelecida.
4. Registro de processo: screenshot do Paint Room do Asset Secundário com painel de Stencils visível. Salvar: `[Nome]_AssetSecundario_S10.3b`.

> **Opção de recuperação:** Estudantes que ainda não receberam nenhum Asset Secundário nesta fase seguem integralmente o Caminho A — não há prejuízo algum em permanecer só no Hero Asset Referência esta semana; o recebimento dos Assets Secundários é escalonado pela disciplina de Modelagem 3D e Level Design, fora do controle do estudante.
...
- **Para quem está no Caminho B:** a linguagem aplicada ao Asset Secundário é reconhecivelmente a mesma do Hero Asset Referência, ou parece um experimento novo e desconectado? *"Coloca os dois lado a lado no Render Room. Alguém que não conhece o seu kit reconheceria que são do mesmo mundo?"*
...
- *"O Asset Secundário é do mesmo material do Hero Asset Referência? Se for, por que o stencil e a cor seriam diferentes?"*
...
**Formato: Walk-around com foco em integração e em coerência de kit**

Esta é uma crítica 🔵 Informal. Não há ficha formal nem apresentação individual estruturada. O objetivo é identificar coletivamente a diferença entre stencil integrado e stencil sobreposto — e, para quem já trabalhou Assets Secundários, verificar se a linguagem visual do kit está começando a se firmar — e definir o que cada estudante deve trabalhar no segundo estúdio.

**Protocolo:**

**Preparação (2 min antes do início):**
- Cada estudante exibe o Hero Asset Referência no Render Room com o detalhe de stencil aplicado e, quando houver, o Asset Secundário correspondente.
- Preparar dois screenshots visíveis: o asset antes do stencil (base da S09) e o estado atual.

**Abertura (2 min):**
*"Vamos olhar os assets com uma pergunta única: o stencil pertence ao material ou está por cima do material? 'Pertencer' significa que o detalhe faz sentido com o tipo de superfície, com a posição no asset, e que as bordas estão integradas. 'Por cima' significa que o detalhe parece colado — as bordas são muito definidas, a cor não dialoga com o material base, ou a posição não tem narrativa. Cada um tem 20 segundos para me dizer: pertence ou está por cima? E por que? Quem já tem Asset Secundário: mais 10 segundos — a linguagem dele conversa com o Hero Asset Referência ou parece de outro kit?"*
...
- Para quem tem Asset Secundário: *"Coloquem o Hero Asset Referência e o Asset Secundário lado a lado. Alguém de fora reconheceria que pertencem ao mesmo kit?"*

Identificar e destacar: um ou dois casos de stencil bem integrado (bordas suavizadas, roughness correspondente, narrativa clara), um ou dois casos que precisam de integração ainda, e — quando houver exemplos — um caso de boa transferência de linguagem visual para o Asset Secundário. Não como crítica negativa — como materialização do próximo passo de cada estudante.

**Síntese (3 min):**
*"O que estou vendo em geral: [X — o que está bem]. O que falta ainda: [Y — o que precisa de atenção]. Nos próximos 60 minutos, quero que cada um resolva pelo menos uma dessas coisas: (1) integrar as bordas do stencil se ainda parecem coladas; (2) aplicar o stencil no canal Roughness se ainda está só no Color; (3) avançar para um segundo detalhe se o primeiro já está resolvido; ou (4), para quem tem Asset Secundário, seguir aplicando a mesma linguagem visual a ele. Saiam desta aula com o Hero Asset Referência em estado de apresentação — semana que vem é Crítica Formal."*
...
> *"Sessenta minutos com foco em qualidade, não em quantidade. Não adicionem stencil novo só para ter mais — refinem o que já existe. Ao final desta sessão, o Hero Asset Referência precisa estar em estado de apresentação: pelo menos um stencil integrado em Color e Roughness, bordas suavizadas, e os mapas exportados para o Blender. Quem já tem um Asset Secundário em andamento: dedique parte do tempo a consolidar a linguagem visual nele também — não precisa estar terminado, mas precisa estar coerente com o Hero Asset Referência. Quem terminar tudo isso com tempo sobrando: aplique os detalhes de edge wear e dirt aprendidos na S09 ao Asset Secundário — o stencil não substitui o desgaste, ele adiciona um nível a mais."*

**Atividade estruturada:**

**Bloco 1 — Refinamento de integração com base no feedback da crítica (≈20 min):**

Para cada ponto levantado na crítica informal, identificar e resolver no Hero Asset Referência (e, quando aplicável, no Asset Secundário):
...
5. **Asset Secundário com linguagem desconectada do Hero Asset Referência:** Revisar paleta de cor e tipo de stencil usado no Asset Secundário, ajustando para corresponder ao que já foi estabelecido no Hero Asset Referência.

**Bloco 2 — Complementar com edge wear e dirt da S09 (≈20 min):**

Para estudantes com o stencil do Hero Asset Referência integrado e resolvido:
1. No canal Color: camada `EdgeWear_Color` com as arestas mais expostas — no Hero Asset Referência, se ainda houver lacunas, ou no Asset Secundário, se já disponível. Lógica de uso da S09.
...
> **Nota de continuidade:** Estudantes que chegaram mais avançados podem ter, ao final desta semana, o Hero Asset Referência com stencil + desgaste completo e um Asset Secundário já com a linguagem visual iniciada. Isso coloca esses estudantes em posição confortável para a Crítica Formal da S11, onde o foco será bake — eles chegam com o kit já ganhando identidade visual de conjunto. Estudantes que ainda não receberam Asset Secundário não estão atrasados: a CF3 avalia o Hero Asset Referência e/ou o primeiro asset do kit disponível, conforme o que cada um tiver em mãos.

**Bloco 3 — Exportação e verificação no Blender (≈15 min):**
1. No 3D Coat: `File → Export Textures`.
2. Exportar Albedo (Color), Roughness, Metallic do Hero Asset Referência (e do Asset Secundário, se trabalhado).
3. Pasta de destino: `[Nome]_HeroAsset_Mapas_S10/` (e `[Nome]_AssetSecundario_Mapas_S10/`, se aplicável).
4. Abrir o Blender com o material do Hero Asset Referência (e do Asset Secundário, se aplicável).
...
7. Capturar render do(s) asset(s) com material S10 conectado.
8. Salvar: `[Nome]_HeroAsset_Blender_S10.blend` (e `[Nome]_AssetSecundario_Blender_S10.blend`, se aplicável).
...
- Para estudantes com Asset Secundário em mãos, mas inseguros sobre reaproveitar o mesmo stencil: *"Vocês não precisam reinventar. O Hero Asset Referência já resolveu essa decisão — que tipo de rachadura, que cor de interior exposto, que nível de desgaste. Reaproveitar essa decisão no Asset Secundário não é preguiça, é coerência de kit — é exatamente isso que o Texture Atlas da Semana 13 vai exigir."*
...
*"Hoje vocês aprenderam que o stencil é uma ferramenta de precisão — não de conveniência. Um stencil mal posicionado, sem justificativa, e sem integração de bordas, prejudica o material em vez de enriquecer. Um stencil bem colocado conta a história de um evento específico: a rachadura que surgiu de um impacto, a corrosão que se instalou em um ponto de umidade, o símbolo gravado por uma mão. A técnica é simples. A decisão é o que importa. E, para quem já começou a trabalhar o primeiro Asset Secundário: hoje foi o dia em que o Kit Modular de vocês começou, de verdade, a existir como conjunto — não mais como uma peça isolada."*

2. **(3 min — Reflexão de processo)**
Pergunta aberta: *"Qual foi o momento de maior dúvida de hoje? Quando vocês não sabiam se o detalhe estava integrado ou não? Ou, para quem trabalhou o Asset Secundário: quando vocês não sabiam se a linguagem visual estava conversando com o Hero Asset Referência? Como vocês resolveram?"*
...
*"Semana que vem é a Crítica Formal 3 — CF3. O tema técnico será Bake: vocês vão aprender a transferir detalhe de um mesh de alta resolução para o low poly, aplicado a um asset do Kit Modular — provavelmente um Asset Secundário, com o par low-poly/high-poly que Modelagem 3D preparou. Mas antes de entrar nesse conteúdo, a crítica vai exigir que o Hero Asset Referência esteja apresentável — com stencils integrados e desgaste. Isso é o que vocês têm desta sessão para amanhã. Quem chegar na S11 com o Hero Asset Referência em estado incompleto vai entrar na parte mais técnica da disciplina com um passivo. Priorizem a exportação e o Blender antes de encerrar."*

*"Para a crítica formal da S11, trazer: Hero Asset Referência (completo, com stencils da S10), Asset Secundário (se já recebido e trabalhado), renders de ambos, pasta de camadas organizada e autoavaliação da rubrica preenchida. A ficha de autoavaliação estará disponível no servidor antes da próxima aula."*
...
**4. Asset Secundário ainda não recebido — estudante sem material para o Caminho B**
Alguns estudantes ainda não receberam Assets Secundários de Modelagem 3D e Level Design nesta fase — isso é esperado e não é atraso do estudante. Estratégia: esses estudantes seguem integralmente o Caminho A (Hero Asset Referência), aprofundando com um segundo stencil ou variação. Não forçar a criação de um Asset Secundário provisório apenas para cumprir a atividade — o objetivo pedagógico do Caminho B é a transferência de linguagem, que só faz sentido quando há uma peça real do kit para receber essa linguagem.

**5. Linguagem visual do Asset Secundário desconectada do Hero Asset Referência**
O estudante trata o Asset Secundário como um projeto novo, escolhendo stencil, paleta e intensidade de desgaste diferentes do que já foi estabelecido. O resultado são duas peças que não parecem do mesmo kit. Estratégia: antes de aplicar qualquer stencil ao Asset Secundário, o estudante deve abrir o Hero Asset Referência ao lado e nomear em voz alta: *"Que stencil eu usei lá? Que cor de interior exposto? Que intensidade de roughness?"* Só depois de responder, aplicar a mesma decisão ao Asset Secundário.

**6. Stencil posicionado aleatoriamente — sem intenção compositiva**
...
**7. Confusão entre stencil e alpha durante o estúdio**
...
**8. Detalhe de stencil desaparece ou distorce no Blender após exportação**
...
| Estudante ainda sem Asset Secundário se sente "atrasado" em relação aos colegas | Esclarecer que o recebimento de Assets Secundários é escalonado por Modelagem 3D e Level Design, fora do controle do estudante. Reforçar que aprofundar o Hero Asset Referência com um segundo detalhe de stencil é tempo bem investido — a CF3 avalia o que cada um tiver disponível. |
| Estudante com Asset Secundário que trata como projeto novo, sem relação com o Hero Asset Referência | *"Abre os dois lado a lado no Render Room. O que é igual? O que é diferente? Se a resposta for 'tudo diferente', ainda não é um kit — são duas peças soltas. Comece copiando a decisão de stencil e paleta do Hero Asset Referência, depois adapte só o necessário."* |
| Estudante avançado que dominou o stencil rapidamente | Propor desafio: criar um stencil personalizado do zero no Krita com um elemento específico do tema (ex: uma runa medieval, um símbolo de facção sci-fi, um logotipo industrial). O stencil criado pelo estudante é mais valioso como portfólio do que um stencil de biblioteca — e pode ser reaproveitado nos próprios Assets de Repetição na Semana 14. |
| Estudante com Hero Asset Referência em estado muito incompleto | Reduzir o escopo para a semana: uma camada base de Color + um stencil simples integrado + exportação. Melhor um detalhe bem resolvido do que três detalhes fragmentados. A Crítica Formal da S11 avalia processo e qualidade, não quantidade. |
...
|---|---|---|
| Camada `Stencil_[Nome]_Color` no arquivo `.3b` do Hero Asset Referência com detalhe visível e posicionado com intenção narrativa | C5 — Texturização | O detalhe existe? A posição tem justificativa de uso (ponto de tensão, elemento temático)? |
...
| Para quem já recebeu Asset Secundário: mesma linguagem de stencil, paleta e desgaste aplicada a ele | C2 — Direção Artística | O Asset Secundário é reconhecivelmente do mesmo kit que o Hero Asset Referência, ou parece um experimento desconectado? |
...
| Mapas do(s) asset(s) trabalhado(s) exportados e conectados corretamente no Blender com Color Space correto | C4 — Materiais PBR | Os três mapas foram exportados? O Roughness tem Color Space = Non-Color? O detalhe do stencil aparece no viewport do Blender? |
...
|---|---|---|
| Arquivo do 3D Coat com o Hero Asset Referência com ao menos um stencil integrado em dois canais | `.3b` ou `.3dc` com sufixo `_HeroAsset_S10` | Até o fim do segundo encontro |
| Mapas PBR exportados do Hero Asset Referência: Albedo, Roughness, Metallic | `.png` 1024×1024 (ou 2048) na pasta `_HeroAsset_Mapas_S10` | Até o fim do segundo encontro |
| Arquivo Blender do Hero Asset Referência com mapas da S10 conectados | `.blend` com sufixo `_HeroAsset_Blender_S10` | Até o fim do segundo encontro |
| Para quem já recebeu Asset Secundário: arquivo `.3b` com a mesma linguagem visual de stencil iniciada, e mapas exportados | `.3b`/`.3dc` com sufixo `_AssetSecundario_S10` e mapas em `_AssetSecundario_Mapas_S10` | Até o fim do segundo encontro (quando aplicável) |
...
> **Nota sobre a Crítica Formal da Semana 11:** O Hero Asset Referência da S10 será apresentado na Crítica Formal da S11 junto com o Asset Secundário (se já recebido e trabalhado). Estudantes que ainda não receberam Asset Secundário apresentam apenas o Hero Asset Referência — sem prejuízo, já que o recebimento é escalonado por Modelagem 3D e Level Design. A ficha de autoavaliação da rubrica mestre deve ser preenchida antes da S11 — ela estará disponível no ambiente virtual antes da próxima aula.
