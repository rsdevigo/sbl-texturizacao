...
- Um Asset Secundário do Kit Modular (ou a peça alternativa prevista para quem ainda não havia recebido nenhum) já com bake de Normal + AO integrado ao material no 3D Coat, convivendo com o desgaste e os stencils já construídos no Hero Asset Referência (S09/S10) — Semana 11
...
> **Nota de transição:** A Semana 11 resolveu o problema de transferir **geometria real** (chanfros, entalhes) de um par high-poly/low-poly para o Asset Secundário via Normal Map e AO. Esta semana não introduz um novo processo de bake — o fluxo de seleção de objetos, cage e execução já foi aprendido. O que muda é o **uso** do bake: ID Map e Curvature Map não existem para adicionar detalhe visual diretamente, mas para gerar **máscaras automáticas** que aceleram a texturização no 3D Coat. É a primeira vez na disciplina em que o bake deixa de ser apenas "captura de aparência" e passa a ser "captura de informação para seleção". Esse é o ponto central da aula: o estudante já sabe bakear — agora aprende a bakear *para outro propósito*.
>
> O alvo desta semana é o **pacote completo de bake (4 mapas)** de um asset do kit — Normal e AO da Semana 11, mais ID Map e Curvature de hoje. Para quem já recebeu mais de um Asset Secundário de Modelagem 3D e Level Design, o ideal é diversificar e aplicar o pacote completo a um **segundo** Asset Secundário, diferente do trabalhado na Semana 11 — isso adianta o kit rumo à Semana 13 (Texture Atlas), que vai exigir múltiplos assets já texturizados. Para quem ainda tem apenas um Asset Secundário (ou trabalhou com a peça alternativa da S11), o pacote completo de quatro mapas é aplicado a essa **mesma** peça, completando-a tecnicamente antes de seguir adiante. Os dois casos são igualmente válidos — o objetivo pedagógico é dominar ID Map e Curvature, não necessariamente abrir uma peça nova.
...
2. Configurar um bake de **ID Map** atribuindo cores distintas por material ou grupo de faces no Asset Secundário trabalhado.
...
5. Comparar o resultado de uma máscara gerada automaticamente por bake com o desgaste pintado manualmente no Hero Asset Referência (Semanas 9 e 10), avaliando onde a automação economiza tempo sem comprometer a intenção artística.
...
| C5 — Texturização | obs. | O detalhe gerado por máscara automática (curvature/ID) dialoga com o desgaste já pintado no Hero Asset Referência, ou parece mecânico e descolado? |
...
- Arquivo `.blend` do Asset Secundário (ou peça alternativa) de cada estudante, já com pacote de bake de Normal + AO da Semana 11; e, para quem tem mais de um Asset Secundário recebido, o arquivo do segundo Asset Secundário a ser trabalhado nesta semana
...
> **Verificação prévia da situação de cada estudante:** Antes da aula, checar rapidamente quantos Assets Secundários cada estudante já recebeu de Modelagem 3D e Level Design. Isso define se o pacote completo de hoje recai sobre um segundo asset (diversificando o kit) ou sobre o mesmo asset da Semana 11 (completando-o) — ver nota de transição acima. Não é necessário resolver isso antes do Encontro 1; a Etapa 1 da Produção em Estúdio já orienta o estudante a decidir.
...
**1. Panorama dos cinco tipos de bake — retomada rápida**

Relembrar em um minuto os cinco tipos apresentados na Semana 11 — Normal Map, Ambient Occlusion, Curvature, ID Map e Thickness — e onde a turma está: Normal e AO já dominados e aplicados em produção; Curvature e ID Map são o foco de hoje; Thickness permanece apenas conceitual, fora do escopo da disciplina.

**2. ID Map (Material ID): mapa de seleção por cor**
...
**3. Curvature Map: leitura automática de arestas e sulcos**
...
*"Lembrem do desgaste pintado à mão no Hero Asset Referência: vocês decidiram região por região onde o EdgeWear fazia sentido. O Curvature Map faz uma primeira passada automática disso — ele já sabe, geometricamente, onde estão as arestas e os sulcos do modelo. Vocês continuam no controle da decisão artística, mas não partem mais do zero."*

**4. Por que essas máscaras aceleram o 3D Coat sem substituir a pintura**

No 3D Coat, o Curvature Map importado pode alimentar diretamente os geradores de **Edge Highlight** (realce de aresta) e **Cavity Dirt** (sujeira de cavidade) — efeitos que, no Hero Asset Referência, foram construídos manualmente com pincéis e alphas. O ID Map alimenta seleções rápidas por material, permitindo isolar "só o metal" ou "só a madeira" no Asset Secundário com um clique, para aplicar um ajuste de cor, roughness ou desgaste específico daquele material.
...
**5. Relação com os parâmetros já dominados na Semana 11**
...
5. *"Comparem mentalmente com a AO da semana passada: a AO escurece onde falta luz ambiente; a Curvature escurece onde a geometria é côncava, independente de luz nenhuma. São informações diferentes, mesmo que visualmente pareçam parecidas à primeira vista."*

**Passo 3 — Importação do pacote completo no 3D Coat (5 min):**
1. Alternar para o 3D Coat com o asset já texturizado (material PBR coerente com o kit + Normal/AO da S11).
...
> *"Cinquenta minutos com dois objetivos: primeiro, gerar o ID Map de um asset do kit que tenha pelo menos dois materiais distintos — se nenhum dos seus assets tiver isso ainda, aproveitem para separar rapidamente uma área que deveria ser de material diferente (ex: reforço metálico numa peça de madeira). Segundo, gerar o Curvature Map do mesmo asset. Se vocês já receberam mais de um Asset Secundário de Modelagem 3D, priorizem trabalhar em um asset **diferente** do que recebeu o bake de Normal e AO na semana passada — assim o kit ganha mais peças texturizadas até a Semana 13. Se vocês só têm um Asset Secundário até agora (ou trabalharam com a peça alternativa da S11), completem o pacote nessa mesma peça. No fim, vocês terão o pacote completo de quatro bakes nesse asset: Normal e AO da semana passada, mais ID Map e Curvature de hoje."*

**Atividade estruturada:**

**Etapa 0 — Definir qual asset receberá o pacote (≈2 min, decisão rápida antes da Etapa 1):**
1. Verificar quantos Assets Secundários já foram recebidos de Modelagem 3D e Level Design.
2. Se houver mais de um: escolher um **diferente** do trabalhado na Semana 11.
3. Se houver apenas um (ou se o trabalho da S11 foi feito na peça alternativa): manter a mesma peça e completar o pacote nela.

**Etapa 1 — Organização de materiais para ID Map (≈12 min):**
1. No asset escolhido na Etapa 0, identificar o maior potencial de separação de material (ex: madeira + metal, pedra + musgo, painel + reforço).
...
1. Sem necessidade de recriar o par high/low: bakear diretamente a partir do low-poly (ou do high-poly da S11, se o detalhe relevante estiver só nele e o asset for o mesmo).
...
**Etapa 4 — Registro de processo (≈8 min):**
1. Salvar o arquivo: `[Nome]_AssetSecundario_BakeCompleto_S12.blend` (usar `_AssetSecundario2_` no nome quando for um segundo asset distinto do da S11, para deixar claro qual peça é qual no acompanhamento do kit).
...
- **A escolha do asset (mesmo vs. diferente da S11) foi feita corretamente conforme a situação real de recebimento de peças do estudante?** Confirmar na Etapa 0 antes que o estudante avance, para não haver retrabalho.
...
- *"Olhando o Curvature Map, você concorda com onde ele marcou as arestas mais claras? Faz sentido com onde já existe desgaste no Hero Asset Referência, ou aponta um lugar que você ainda não tinha notado?"*
...
**Síntese (2 min):**
*"A maior parte de vocês já tem o pacote completo de quatro bakes em pelo menos um asset do kit. O trabalho de hoje no estúdio é usar essas duas novas máscaras dentro do 3D Coat para acelerar a texturização — não é para substituir o que vocês já pintaram, é para dar um empurrão automático antes de refinar à mão."*
...
2. Comparar com o Dirt pintado manualmente no Hero Asset Referência: ajustar a opacidade da camada automática para que ela **reforce**, e não duplique ou contradiga, a linguagem visual já construída no kit.
...
- Para estudantes que terminam rápido: sugerir aplicar o mesmo fluxo (Curvature + ID Map) em outro asset do kit ainda sem pacote completo, mantendo coerência entre todas as peças já texturizadas.
...
*"Hoje vocês fecharam o pacote completo de bake da disciplina: Normal, AO, Curvature e ID Map, aplicado a um asset do Kit Modular. A partir de agora, qualquer asset novo que vocês trabalharem pode passar por esse fluxo completo desde o início, em vez de aprender cada mapa separadamente."*
...
Deixar 2–3 respostas. O objetivo é consolidar que ferramentas automáticas em produção de jogos aceleram, mas não substituem, o julgamento artístico — um tema que volta a aparecer nas semanas seguintes com Texture Atlas e Trim Sheets.

3. **(2 min — Antecipação da Semana 13)**
*"Semana que vem: Texture Atlas — vamos combinar pelo menos três assets do kit em uma única textura, incluindo o Hero Asset Referência e os Assets Secundários que vocês já receberam de Modelagem 3D e Level Design. Antes de chegar: pensem em quais peças do seu kit poderiam, plausivelmente, compartilhar a mesma textura sem perder qualidade visual — o pacote de bake que vocês fecharam hoje é parte do que entra nesse atlas."*
...
Hábito reforçado pela Semana 11, em que high/low era sempre necessário. Estratégia: lembrar diretamente que Curvature funciona a partir da geometria existente do próprio low-poly (ou do high-poly da S11, se já existir no mesmo asset e tiver detalhe relevante) — não é necessário duplicar objetos novamente.
...
O Cavity Dirt gerado automaticamente pode aparecer em uma área onde o estudante já havia decidido, artisticamente, que não deveria haver sujeira (ex: uma superfície polida por uso constante, que deveria ficar limpa mesmo sendo uma cavidade geométrica). Estratégia: reforçar que a opacidade da camada automática é sempre ajustável e que o estudante tem autoridade para reduzir ou apagar localmente onde a leitura automática não corresponde à narrativa de uso pretendida do kit — a automação é ponto de partida, não veredito final.
...
**7. Estudante ainda não sabe se deve trabalhar em um asset novo ou no mesmo da Semana 11**
Alguns estudantes podem hesitar entre diversificar (segundo Asset Secundário) ou completar a peça já iniciada, especialmente se a situação de recebimento de peças de Modelagem 3D mudou desde a semana passada. Estratégia: resolver isso logo na Etapa 0 da Produção em Estúdio, com uma checagem rápida e objetiva — mais de um Asset Secundário disponível: escolher um diferente do da S11; apenas um disponível: completar o mesmo. Não é uma decisão que exige tempo de reflexão.
...
| Estudante inseguro sobre onde aplicar Cavity Dirt automático | Pedir para comparar com o Dirt pintado manualmente no Hero Asset Referência: "Essas duas leituras concordam? Se sim, você pode confiar na automática como ponto de partida. Se não, qual delas está mais de acordo com a história de uso do seu objeto?" |
| Estudante terminando rápido e com qualidade | Propor aplicar o pacote completo de quatro bakes em outro Asset Secundário do kit ainda sem esse pacote, mantendo a mesma lógica de nomenclatura de material usada no primeiro — reforça consistência entre as peças do kit (C2). |
...
| Estudante inseguro se deve trabalhar no mesmo asset da S11 ou em um novo | Reforçar que os dois caminhos são igualmente válidos e igualmente avaliados: o que importa esta semana é o domínio de ID Map e Curvature, não qual peça específica recebe o pacote. Se houver mais de um Asset Secundário disponível, diversificar é preferível — mas não é penalizado completar a mesma peça quando só há uma disponível. |
...
| Pacote completo de quatro bakes (Normal, AO, Curvature, ID Map) de um asset do kit, organizado com nomenclatura consistente | C1 — Processo de Projeto | Os arquivos estão nomeados de forma que qualquer pessoa entenda o que cada mapa representa e a qual asset pertence, sem abrir o arquivo? |
| Cavity Dirt ou Edge Highlight gerado via Curvature, integrado como camada adicional (não substitutiva) sobre o desgaste e os stencils já construídos no Hero Asset Referência | C5 — Texturização | O resultado automático reforça a narrativa de uso já construída no kit, ou contradiz decisões artísticas anteriores sem ajuste? |
...
|---|---|---|
| Arquivo Blender/3D Coat com pacote completo de bake (Normal, AO, Curvature, ID Map) do asset do kit trabalhado, integrado ao material | `.blend` e projeto 3D Coat com sufixo `_AssetSecundario_BakeCompleto_S12_Final` (usar `_AssetSecundario2_` quando for um segundo asset distinto do da S11) | Até o fim do segundo encontro |
