...
**Crítica:** 🔴 **FORMAL — CF3** (25% do Portfolio de Artefatos) — Apresentação estruturada com rubrica, autoavaliação obrigatória e feedback escrito do professor
...
- Hero Asset Referência com stencil(s) integrado(s) e revisão de desgaste concluída — Semana 10
- Para quem já recebeu Assets Secundários de Modelagem 3D e Level Design: início da aplicação da mesma linguagem visual (material PBR coerente com o kit, UV simples) a essas peças — Semana 10
...
> **Nota de transição — entrando na Unidade III:** A partir da Semana 10, o Kit Modular deixou de ser um conceito futuro e passou a ter peças reais em produção. O estudante trabalha agora com três papéis de asset distintos, cada um com seu próprio ciclo: o **Hero Asset Referência** (a peça desenvolvida em profundidade nas Semanas 4–9, que já passou por UV completo, PBR, bake de mesh maps e texturização artística — seu ciclo de desenvolvimento intensivo está concluído, e ele será incorporado ao kit como uma peça entre outras a partir da Semana 13); os **Assets Secundários** (2 a 3 peças adicionais entregues por Modelagem 3D e Level Design, com ciclo de produção abreviado — UV simples e material PBR coerente com o kit); e os **Assets de Repetição** (elementos modulares que se repetem, como paredes, vigas ou molduras, que só entram em produção a partir da Semana 14 via Trim Sheet). Esta semana, o bake de Normal Map, AO e Curvature é aplicado a **um Asset Secundário** — é nesse tipo de peça que faz sentido pedagógico preparar um par low-poly/high-poly novo, já que o Hero Asset Referência não tem um par assim pronto (seu Normal já nasceu de pintura de Depth direta na malha de produção, desde a Semana 7). O Normal Map de alta fidelidade obtido hoje convive, no material do Asset Secundário, com os mesmos princípios de PBR e desgaste já dominados no Hero Asset Referência — o que muda é a peça sendo trabalhada, não a técnica de fundo.

> **Se você ainda não recebeu nenhum Asset Secundário:** Alguns estudantes podem chegar a esta semana sem ter recebido ainda nenhuma peça de Modelagem 3D e Level Design — isso é uma dificuldade prevista, não um bloqueio (ver seção **Possíveis Dificuldades**, item 7). Nesse caso, o bake desta semana é praticado em um **Asset de Repetição simples** (se já houver algum disponível, mesmo que ele só entre oficialmente em produção via Trim Sheet na Semana 14) ou, como último recurso, revisitando o **Hero Asset Referência** com uma malha auxiliar de alta resolução simplificada, preparada pelo professor especificamente para esse fim. O objetivo pedagógico da semana — dominar o fluxo técnico de bake — se sustenta em qualquer um dos três casos; o que muda é apenas qual peça recebe o resultado.

> **Atenção — Crítica Formal (CF3):** Esta é a 3ª das 5 críticas formais da disciplina e a primeira em que o Critério 6 (Bake) entra em avaliação. C4 (Materiais PBR) e C5 (Texturização), já avaliados desde CF1/CF2, são reavaliados sobre o estado atual do kit como um todo (Hero Asset Referência + Assets Secundários em andamento), e C9 (Apresentação) passa de observação para nota formal. O professor deve reservar tempo estruturado para a apresentação individual — esta não é uma crítica circulante como nas semanas anteriores.
...
2. Identificar os tipos de mapa de bake mais usados em produção de jogos — **Normal Map**, **Ambient Occlusion**, **Curvature**, **ID Map** e **Thickness** — e reconhecer para que serve cada um (o foco produtivo desta semana é Normal e AO; Curvature é apresentada conceitualmente e aprofundada, junto com ID Map, na Semana 12).
...
4. Executar um bake de Normal Map e de Ambient Occlusion sem artefatos visíveis (sem manchas em superfícies planas, sem vazamento — *bleeding* — entre ilhas de UV) em um Asset Secundário do Kit Modular (ou alternativa prevista, conforme a nota acima).
5. Integrar o Normal Map e o AO baked ao material do asset trabalhado no 3D Coat, mantendo coerência de linguagem visual com o Hero Asset Referência.
6. Apresentar formalmente o estado atual do Kit Modular na Crítica Formal 3, justificando decisões técnicas e artísticas com base nos critérios da rubrica.
...
| C2 — Direção Artística | ✓ Nota | Coerência visual entre o Hero Asset Referência e o(s) Asset(s) Secundário(s) em andamento; o bake reforça ou compromete a identidade visual do kit? |
| C3 — UV Mapping | — Não aplicável nesta CF | — |
| C4 — Materiais PBR | ✓ Nota | Os canais PBR permanecem fisicamente plausíveis após a integração do Normal Map de bake? O bake de hoje é coerente com o Normal via Depth já dominado no Hero Asset Referência desde a Semana 7? |
| C5 — Texturização | ✓ Nota | O detalhe de bake se soma à narrativa de desgaste já construída no kit, ou parece desconectado do restante? |
...
- Arquivos `.blend` do Hero Asset Referência (com material completo desde a Semana 9/10) e do(s) Asset(s) Secundário(s) já recebido(s) de Modelagem 3D e Level Design, de cada estudante
- Para estudantes ainda sem Asset Secundário: arquivo de malha auxiliar de alta resolução simplificada, preparada previamente pelo professor a partir do Hero Asset Referência de cada estudante (ou um Asset de Repetição simples já disponível), para servir de high-poly de contingência
...
> **Preparação da malha auxiliar de contingência:** Antes da aula, verificar quais estudantes ainda não receberam Asset Secundário de Modelagem 3D e Level Design. Para eles, preparar (ou reservar tempo para preparar durante o Encontro 1) uma malha auxiliar de alta resolução simplificada derivada do próprio Hero Asset Referência — não precisa ser elaborada, apenas suficiente para gerar um par high-poly/low-poly funcional que permita a esses estudantes praticar o fluxo completo de bake sem ficar bloqueados esperando a entrega de Modelagem 3D.
...
O Normal Map de bake do Asset Secundário segue a mesma lógica de PBR e desgaste já dominada no Hero Asset Referência — ele não é uma técnica isolada, é a mesma linguagem visual do kit aplicada a uma nova peça. No 3D Coat, o Normal Map baked entra no canal Normal do material do asset, e o EdgeWear/Dirt são pintados por cima ou ao redor dele para que a peça nova converse visualmente com o Hero Asset Referência.
...
**Criação de detalhe high-poly e execução do bake em um Asset Secundário**

**Consigna entregue verbalmente:**

> *"Cinquenta minutos com um objetivo: escolher um Asset Secundário do kit — ou uma parte dele — que se beneficiaria de detalhe de geometria real: um chanfro, um entalhe, uma sobreposição de placas. Criar uma versão high-poly dessa parte e executar o bake de Normal Map e AO para o low-poly de produção. Se você ainda não recebeu nenhum Asset Secundário de Modelagem 3D, use a malha auxiliar de contingência preparada para você, ou um Asset de Repetição simples se já houver um disponível — o objetivo de hoje é dominar o fluxo técnico de bake, e para isso qualquer um desses pares serve. Não precisa ser a peça inteira: pode ser só uma parte. O foco é o processo, não cobrir o asset inteiro de detalhe."*

**Atividade estruturada:**

**Etapa 1 — Seleção da área e planejamento (≈10 min):**
1. Abrir o arquivo `.blend` do Asset Secundário recebido (ou da malha de contingência, para quem ainda não recebeu nenhum).
...
2. No material do low-poly: criar os dois Image Texture Nodes de destino (`_NormalBake` e `_AOBake`), resolução coerente com os demais mapas do kit.
...
1. Salvar o arquivo: `[Nome]_AssetSecundario_Bake_S11.blend` (ou `[Nome]_AssetRepeticao_Bake_S11.blend` / `[Nome]_HeroAsset_BakeContingencia_S11.blend`, conforme a peça usada — preservando o arquivo da semana anterior).
...
Atenção especial aos estudantes usando a malha de contingência (sem Asset Secundário ainda recebido): confirmar que entenderam que esse bake é um exercício de processo, válido para a nota de C6, e que será reaplicado ao Asset Secundário real assim que ele chegar — não é trabalho perdido.
...
- *"O Normal Map baked está dialogando com a linguagem visual que você já construiu no Hero Asset Referência? Um chanfro recém-modelado precisa do mesmo tipo de desgaste que você já pintou lá?"*
...
**Formato: Apresentação individual estruturada com rubrica, autoavaliação obrigatória e feedback escrito do professor**
...
- Cada estudante prepara: Hero Asset Referência e o Asset Secundário (ou peça de contingência) trabalhado nesta semana abertos no Blender (viewport Rendered), com os mapas de bake da S11 já conectados; renders comparativos (antes/depois do bake) prontos para exibir.

**Abertura (1 min):**
*"Cada apresentação tem cerca de 2 minutos: mostrem o estado atual do Kit Modular — o Hero Asset Referência e a peça em que vocês aplicaram o bake desta semana — expliquem o que mudou desde a última crítica formal (Semana 8) e destaquem especificamente o que o bake adicionou. Depois, uma pergunta rápida da turma ou minha. Vou preencher a ficha durante a apresentação de vocês — sem afetar o ritmo."*
...
1. Mostrar o Hero Asset Referência: recapitular brevemente o estado atual (desgaste da S09, stencil da S10).
2. Mostrar o Asset Secundário (ou a peça de contingência usada): destacar o high-poly criado e o bake de Normal/AO aplicado, e como a linguagem visual do kit foi mantida.
...
> *"Sessenta minutos com foco na prioridade de melhoria que cada um recebeu na crítica formal de hoje. Se o problema foi um artefato de bake, corrijam o cage e regravem. Se foi falta de integração entre o bake e o desgaste já construído no kit, ajustem isso no 3D Coat. Se o Asset Secundário já está sólido, usem o tempo para aplicar bake em uma segunda área — outra peça do kit, ou outra parte da mesma peça. Quem estava trabalhando com a malha de contingência: se o Asset Secundário real chegou nesse meio-tempo, aproveitem para já iniciar o bake nele."*
...
1. Importar o Normal Map baked como uma camada adicional no canal Normal do 3D Coat do Asset Secundário.
2. Importar o AO baked: usar como camada de Multiply sobre o canal Color, opacidade ajustável (60–80%), para reforçar visualmente as frestas e cantos criados.
3. Verificar no Render Room: o bake reforça ou desconecta da linguagem visual já estabelecida no Hero Asset Referência? Ajustar opacidade das camadas para que o Asset Secundário leia como parte do mesmo kit, e não como uma peça isolada.

**Bloco 3 — Segunda aplicação de bake, se houver tempo (≈15 min):**

Para estudantes com o Asset Secundário já resolvido (ou que receberam um segundo Asset Secundário nesse meio-tempo):
1. Repetir o processo de high-poly → bake em uma segunda área do kit (outro Asset Secundário, ou outra parte da mesma peça).
...
2. Salvar arquivo Blender atualizado: `[Nome]_AssetSecundario_Bake_S11_Final.blend`.
...
- Para estudantes ainda dependentes da malha de contingência: confirmar com Modelagem 3D e Level Design a previsão de entrega do Asset Secundário real, e registrar isso para acompanhamento na Semana 12.
...
*"Semana que vem: bake avançado — ID Map e Curvature, aplicados a um segundo asset do kit. Vocês vão aprender a usar esses dois mapas como base de mascaramento automático no 3D Coat, o que acelera bastante a texturização de detalhe. Antes de chegar: pensem em qual peça do kit teria mais de um material distinto (ex: madeira + metal no mesmo objeto) — esse é o candidato ideal para ID Map."*
...
**5. Bake tecnicamente limpo mas visualmente desconectado do desgaste já pintado no kit**
O Normal Map de bake fica perfeito isoladamente, mas quando combinado com o EdgeWear e o Dirt do Hero Asset Referência, o resultado parece "duas texturas coladas" — o chanfro bakeado do Asset Secundário é nítido demais comparado ao desgaste orgânico do resto do kit. Estratégia: reforçar que a integração continua sendo trabalho de pintura livre — usar uma leve camada de Dirt ou EdgeWear sobre a área recém-bakeada para que ela receba o mesmo tratamento narrativo do restante do kit.
...
**7. Estudante ainda não recebeu nenhum Asset Secundário de Modelagem 3D**
Atrasos na entrega de Modelagem 3D e Level Design são esperados em produções integradas entre disciplinas, e alguns estudantes podem chegar à Semana 11 sem ter nenhum Asset Secundário para bakear. Isso é uma dificuldade prevista, não um bloqueio: o estudante realiza o bake em um Asset de Repetição simples já disponível ou, como último recurso, na malha auxiliar de alta resolução simplificada preparada pelo professor a partir do próprio Hero Asset Referência. Estratégia: identificar esses casos antes da aula (ver **Preparação da malha auxiliar de contingência** em Recursos necessários) para que o estudante não perca tempo de estúdio decidindo o que fazer, e deixar claro que o exercício técnico vale normalmente para C6 — o bake será reaplicado ao Asset Secundário real assim que ele for entregue.
...
| Estudante que termina rápido e com qualidade | Propor um segundo bake em outra parte do kit, ou introduzir uma pergunta antecipada da Semana 12: *"Se essa peça tivesse dois materiais diferentes — por exemplo, madeira e reforço metálico — como você acha que separaria isso automaticamente na textura? Pesquise 'Material ID' e me conte o que encontrou."* |
| Estudante confuso sobre por que o Normal Map de bake e o Normal via Depth da Semana 7 coexistem no kit | Explicar com analogia: *"O Depth da Semana 7, aplicado ao Hero Asset Referência, é como uma textura de pele — repete um padrão geral (poros, rugosidade fina) pintado à mão por toda a superfície. O bake de hoje, no Asset Secundário, é como uma cicatriz específica — um detalhe único, localizado, que só existe naquele ponto exato porque veio de uma geometria real ali. As duas técnicas convivem no mesmo kit porque servem a propósitos diferentes."* |
...
| Estudante sem Asset Secundário recebido, inseguro se o exercício "vale a mesma coisa" | Reforçar explicitamente que o Critério C6 avalia o domínio técnico do processo de bake, não qual peça específica recebeu o bake — o exercício na malha de contingência ou no Asset de Repetição tem o mesmo peso de aprendizagem, e será reaplicado ao Asset Secundário assim que ele chegar. |
...
|---|---|---|
| Objeto high-poly e low-poly presentes no arquivo `.blend` do Asset Secundário (ou peça de contingência), com escala e origem coincidentes | C6 — Bake | A geometria auxiliar existe e está corretamente posicionada em relação ao low-poly de produção? |
...
| Bake integrado de forma coerente à linguagem visual já construída no Hero Asset Referência (desgaste, stencils) | C5 — Texturização | O novo detalhe de geometria dialoga com a narrativa de uso já construída no kit, ou parece um elemento isolado e recém-adicionado? |
| Coerência entre o Hero Asset Referência e o Asset Secundário mantida após a introdução do bake | C2 — Direção Artística | O nível de detalhe geométrico introduzido é proporcional e consistente com o restante do kit? |
...
|---|---|---|
| Arquivo Blender com objeto high-poly e low-poly do Asset Secundário (ou peça de contingência) trabalhado, mais o material atualizado com bake integrado | `.blend` com sufixo `_AssetSecundario_Bake_S11_Final` (ou `_AssetRepeticao_Bake_S11_Final` / `_HeroAsset_BakeContingencia_S11_Final`, conforme a peça usada) | Até o fim do segundo encontro |
