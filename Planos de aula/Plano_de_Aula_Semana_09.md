...
**Tema da semana:** Texturização artística: desgaste, sujeira e detalhes hand-painted no 3D Coat  
...
> **Semana da SCT (Semana de Ciência e Tecnologia):** esta é a semana do evento institucional. O professor deve ajustar o ritmo de estúdio conforme a agenda da SCT no campus — se houver sobreposição de horário ou de atenção da turma, priorizar a Etapa de Edge Wear e Dirt no Encontro 1 e flexibilizar a extensão do refinamento no Encontro 2.
...
- Hero Asset Referência texturizado no 3D Coat (arquivo `.3b` salvo): canais Color, Roughness e Metallic com pelo menos uma camada base e uma camada de variação em cada — Semana 8
- Os 4 mapas PBR exportados (Albedo, Metallic, Roughness, Normal em PNG 1024×1024 ou superior) e conectados corretamente no Blender, sem nós procedurais remanescentes — Semana 8 (CF2)
...
> **Nota de transição:** A Semana 8 fechou a exportação completa do material PBR do Hero Asset Referência e encerrou a Unidade II com a CF2. Os estudantes já têm os quatro mapas fundamentais com camadas base. Esta semana move o foco do **estrutural** para o **narrativo**: o que diferencia uma textura funcional de uma textura com caráter? A resposta é desgaste intencional, sujeira com lógica e variação de cor que conta a história do objeto. O conteúdo novo não é uma nova ferramenta — é uma forma diferente de usar o 3D Coat, agora com propósito artístico claro, e é a última semana da Unidade II antes de a Unidade III abrir, na Semana 10, a fase de Kit Modular.
...
5. Construir variação de sujeira e cor em múltiplas camadas, evitando um resultado uniforme — diferentes intensidades e tons de acúmulo em diferentes regiões do Hero Asset Referência, conforme a lógica de uso de cada uma.
6. Avaliar a leitura do material à distância de uso (não apenas em close-up), verificando se o desgaste contribui para a silhueta e legibilidade do asset.
7. Exportar e atualizar os mapas no Blender com os novos detalhes pintados, e capturar um render comparativo antes/depois.
...
| C5 — Texturização | Qualidade e intenção das camadas de detalhe: edge wear nas arestas corretas, sujeira com lógica gravitacional e de uso, variação de cor entre regiões, scratches com direção e intensidade plausíveis |
...
- Arquivos `.3b` do Hero Asset Referência de cada estudante (3D Coat — Semana 8)
...
Dirt_Color_Var2    ← segunda camada de sujeira, tom/intensidade distinta
...
**4. Variação como recurso — sujeira não é um único tom**

Um erro comum de iniciante é tratar "sujeira" como uma única camada, uniforme, aplicada de uma vez. Sujeira real tem histórico: uma camada mais antiga e espalhada, e depósitos mais recentes e concentrados em pontos específicos. Duas camadas de Dirt com opacidades e tons diferentes — uma mais sutil e ampla, outra mais intensa e localizada — produzem uma leitura muito mais rica do que uma camada única forçada a fazer os dois trabalhos.

**5. A leitura à distância de uso**
...
Regra prática: se o edge wear não aparece no viewport do Blender com câmera a 5 metros de distância, está fraco demais. Se o asset parece "coberto de arranhões" a essa distância, está forte demais. O objetivo é que o desgaste **leia à distância** sem ser caricato — e que a silhueta do objeto continue legível mesmo com todos os detalhes de superfície ativos.
...
**Passo 3 — Dirt no canal Color, em duas camadas de variação (6 min):**
1. Canal ativo: **Color**.
...
6. Criar uma segunda camada `Dirt_Color_Var2`, modo Multiply, opacidade menor (30–40%), com um tom ligeiramente diferente (mais escuro ou com viés de cor distinto — por exemplo mais verde-acinzentado para umidade). Aplicar apenas nos pontos de acúmulo mais extremo (cantos mais profundos, frestas), não na mesma área ampla da primeira camada.
7. *"Repara que o brush de opacidade baixa me deixa construir o acúmulo de forma gradual. A sujeira não é um evento — é um processo, com camadas de tempos diferentes. A pintura também deve ser."*
...
**Passo 5 — Verificação à distância e de silhueta (2 min):**
1. No Render Room: afastar a câmera para simular a distância de jogo.
2. Perguntar para a turma: *"Vocês conseguem ler o edge wear daqui? E a sujeira? E a silhueta do objeto continua clara, ou os detalhes de superfície estão competindo com a forma?"*
...
4. *"A demonstração não termina quando você acha bonito no Render Room de perto. Termina quando você aprova a distância de uso — e quando a silhueta continua legível."*

> **Nota do professor:** Não é necessário demonstrar scratches nesta sessão — deixar para o estúdio como desafio para estudantes avançados. O foco da demonstração são edge wear e dirt (incluindo a variação em duas camadas), que são os detalhes mais difíceis de acertar a lógica. Scratches têm lógica mais simples de inferir.

---

### Produção em Estúdio — 50 minutos

**Adição de camadas de detalhe ao Hero Asset Referência**

**Consigna entregue verbalmente:**

> *"Cinquenta minutos com um objetivo: adicionar desgaste com história ao Hero Asset Referência. Antes de abrir o 3D Coat, olhem o moodboard e respondam mentalmente: 'O que aconteceu com esse objeto? Ele foi muito usado? Abandonado? Ainda está em uso ativo?' A resposta vai ditar onde vocês pintam. Quando terminar, tirem um screenshot do Render Room com a câmera longe — se o desgaste aparecer, está no caminho certo. Se não aparecer, está fraco demais. Não façam desgaste bonito: façam desgaste verdadeiro."*

**Atividade estruturada:**

**Etapa 1 — Revisão do arquivo da Semana 8 (≈5 min):**
1. Abrir o arquivo `.3b` do Hero Asset Referência salvo na Semana 8.
...
**Etapa 3 — Dirt / Grime com variação em camadas (≈20 min):**
...
5. Nova camada `Dirt_Color_Var2`, modo Multiply, opacidade 30%, tom distinto da primeira (mais escuro, ou com viés de cor diferente). Aplicar somente nos pontos de acúmulo mais extremo, dentro da área já marcada pela primeira camada — não espalhar por toda a superfície.

No canal **Roughness**:
1. Nova camada `Dirt_Rough`. Modo: Multiply. Opacidade: 70%.
2. Pintar nas mesmas áreas com valor de roughness maior (cor mais clara).

**Etapa 4 — Verificação de silhueta, distância e registro (≈10 min):**
1. Render Room: câmera afastada para distância de uso. Avaliar a leitura e a silhueta do objeto — os detalhes reforçam a forma ou competem com ela?
2. Screenshot do Render Room com câmera próxima (close-up de detalhe).
3. Screenshot do Render Room com câmera afastada (distância de uso, silhueta visível).
4. Salvar: `[Nome]_HeroAsset_S09.3b`.
...
- **A leitura à distância e de silhueta funciona?** Pedir ao estudante que afaste a câmera no Render Room antes de avançar. Se o edge wear desaparece completamente, ou se a silhueta perde clareza com excesso de detalhe, precisar reforçar ou reduzir.
...
- *"A sujeira está só nas arestas ou também nos cantos e partes inferiores? Sujeira real não sobe — ela cai e acumula. Ela é toda igual, ou tem uma camada mais antiga e uma mais recente?"*
...
- Cada estudante exibe o Hero Asset Referência no Render Room do 3D Coat ou no viewport do Blender com os mapas conectados.
...
- *"A sujeira está onde a gravidade depositaria ela? Ou está distribuída uniformemente? Dá para perceber mais de uma 'camada' de sujeira, ou parece um único tom aplicado de uma vez?"*
- *"Compara o antes e o depois. O que mudou de mais significativo? O que ainda está igual?"*
- *"A silhueta do objeto ainda está clara com todo esse detalhe, ou o excesso de pintura está confundindo a leitura da forma?"*

Identificar e destacar para a turma: um ou dois exemplos de edge wear com lógica de uso clara, e um ou dois exemplos de Dirt com modo de mistura, variação em camadas e lógica gravitacional bem aplicados. Tornar visíveis os bons exemplos — não para criar ranking, mas para materializar o que "nível 4" parece na prática.

**Síntese (3 min):**
*"O que a turma está acertando: [X]. O que ainda precisa de atenção: [Y]. Nos próximos 60 minutos, quero que cada um refine ao menos uma coisa que os colegas apontaram. Não é hora de adicionar mais coisa em qualquer lugar — é hora de intensificar o que já existe, aprofundar a variação de sujeira, ou corrigir o que está errado na lógica."*
...
> *"Sessenta minutos divididos assim: 35 minutos para refinamento, variação de sujeira e scratches; 15 minutos para exportação e atualização do Blender; 10 minutos finais de checagem cruzada com um colega. O objetivo hoje não é começar coisa nova — é fazer o Hero Asset Referência ficar irretocável nos detalhes que já existem: silhueta legível, sujeira com camadas de intensidade, edge wear com lógica de uso."*
...
- Verificação de silhueta: reduzir intensidade de detalhes que estejam competindo com a leitura da forma

**Bloco 2 — Aprofundamento da variação de sujeira e adição de scratches (≈20 min):**

Para estudantes que completaram edge wear e dirt com lógica clara:

No canal **Color**, aprofundar a variação:
1. Revisar a `Dirt_Color_Var2` já iniciada no Encontro 1: ajustar intensidade e extensão para garantir que ela se distingue claramente da `Dirt_Color` de base, sem virar uma segunda camada idêntica.
2. Opcional, para quem tem tempo: uma terceira variação pontual (por exemplo, uma mancha de umidade ou oxidação localizada, modo Overlay, opacidade baixa) em um único ponto de destaque do asset — não espalhar por toda a superfície.
...
3. Pasta de destino: `[Nome]_HeroAsset_Mapas_S09/` (nova pasta — preservar a da Semana 8 para comparação).
...
8. Salvar: `[Nome]_HeroAsset_3DCoat_S09.blend`.

**Bloco 4 — Checagem cruzada entre colegas (≈10 min):**

Para todos, ao final do fluxo:
1. Trocar de computador com um colega (ou mostrar a tela) por 2–3 minutos.
2. O colega observa o Render Room à distância de uso e responde: *"O que eu vejo de história nesse objeto? Onde eu ainda não sei explicar o que aconteceu com ele?"*
3. Anotar rapidamente o retorno — será usado como ponto de partida da autoavaliação da CF3.
...
- Para estudantes com Dirt em uma única camada uniforme: *"Sua sujeira parece ter acontecido em um dia só, ou em anos? Cria uma segunda camada mais escura e mais localizada só nos pontos extremos — isso já muda a percepção de tempo."*
- Para estudantes avançados com tempo sobrando: *"Você pode criar uma variação de cor no canal Color que simula umidade — uma segunda ou terceira camada de Dirt com cor levemente azulada nas partes mais côncavas e protegidas, modo Overlay, opacidade 20%. Isso dá a impressão de umidade acumulada. Testa e vê se faz sentido para o seu tema. Ou refina a leitura de silhueta: afasta a câmera ao máximo e pergunta se ainda dá para reconhecer a forma do objeto em 2 segundos."*
...
*"Hoje vocês passaram de 'textura com canais corretos' para 'textura com história'. Essa é a diferença entre um asset funcional e um asset que comunica algo. Edge wear, dirt em camadas de variação, scratches — esses não são detalhes decorativos. São respostas às perguntas: 'Quem usou esse objeto? Como? Por quanto tempo?' Quando vocês sabem responder isso, o desgaste aparece nos lugares certos automaticamente — e a silhueta continua contando a história junto com a textura."*
...
3. **(3 min — Antecipação da Semana 10 e do recesso)**
*"Na semana que vem: stencils. Vocês vão aprender a usar máscaras para aplicar padrões específicos com precisão no Hero Asset Referência — rachaduras em pedra, símbolos entalhados, corrosão química, grafites. Stencils permitem detalhe que pintura livre não consegue com facilidade. A Semana 10 também é o início da fase de Kit Modular: quem já recebeu os primeiros Assets Secundários de Modelagem 3D vai começar a aplicar a mesma linguagem visual a eles. Para chegar preparado: pensem em que tipo de detalhe específico o tema do kit pede que não dá para pintar à mão com naturalidade. Esse é o candidato perfeito para stencil. E um lembrete de calendário: logo depois da Semana 10 temos o recesso do Saco Cheio (12 a 16/10) — sem aula nesse período."*
...
**3. Sujeira em camada única, sem variação de tempo ou intensidade**
O estudante pinta toda a sujeira em uma única camada, com uma única opacidade e um único tom, o que faz o objeto parecer sujo "de um dia só" em vez de acumular sujeira ao longo do tempo. Estratégia: exigir explicitamente uma segunda camada de Dirt com tom ou opacidade diferente, aplicada apenas nos pontos de acúmulo mais extremo — a combinação de uma camada ampla e sutil com uma camada concentrada e mais intensa já produz uma leitura de tempo muito mais convincente.

**4. Desgaste invisível à distância de uso**
...
**5. Excesso de detalhe compromete a leitura de silhueta**
Estudante empolgado com o número de camadas de detalhe acaba cobrindo o asset inteiro de desgaste, sujeira e scratches, e a forma do objeto perde clareza — tudo "grita" ao mesmo tempo. Estratégia: no Render Room, afastar a câmera ao ponto de silhueta (contra-luz ou baixa saturação) e perguntar se a forma ainda é reconhecível em 2 segundos. Se não for, reduzir a opacidade geral das camadas de detalhe ou restringir sua área de aplicação.

**6. Direção dos scratches sem lógica de uso**
...
**7. Comparativo antes/depois sem diferença visível**
...
| Estudante avançado que terminou o Hero Asset Referência com qualidade | Propor exploração de variação de cor com umidade: segunda ou terceira camada de Dirt com azul-acinzentado frio, modo Overlay, opacidade 15–20% em cavidades profundas. Ou: refinar a leitura de silhueta em contra-luz, garantindo que a forma se mantém legível mesmo com todos os detalhes ativos. |
...
| Camadas de Dirt_Color, Dirt_Color_Var2 e Dirt_Rough com modo Multiply e lógica gravitacional (partes inferiores e côncavas) | C5 — Texturização | O Dirt está nos locais corretos (partes baixas, côncavas, protegidas)? O modo Multiply está ativo? Existe variação perceptível entre camadas de sujeira (não é um tom único)? A cor de sujeira é coerente com o tema do kit? |
| Silhueta do Hero Asset Referência legível à distância mesmo com todas as camadas de detalhe ativas | C5 — Texturização | A forma do objeto continua reconhecível em contra-luz ou baixa saturação, sem que o excesso de detalhe confunda a leitura? |
...
|---|---|---|
| Arquivo do 3D Coat com o Hero Asset Referência com camadas de detalhe (EdgeWear, Dirt com variação e, se houver tempo, Scratches) | `.3b` ou `.3dc` com sufixo `_HeroAsset_3DCoat_S09` | Até o fim do segundo encontro |
...
| Arquivo Blender atualizado com mapas da S09 conectados | `.blend` com sufixo `_HeroAsset_3DCoat_S09` | Até o fim do segundo encontro |
...
> **Nota:** Não há entrega adicional relacionada a outros assets do kit nesta semana — o foco permanece integralmente no Hero Asset Referência. A partir da Semana 10 começa a fase de Kit Modular, com os primeiros Assets Secundários entrando no fluxo de texturização para quem já os recebeu de Modelagem 3D e Level Design.
